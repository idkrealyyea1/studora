...
function openModal(service) {
  ...
  document.getElementById("modalDescription").textContent = service.longDescription || service.description;

  const deliveryEl = document.querySelector("#modalDelivery span");
  if (deliveryEl) deliveryEl.textContent = service.deliveryTime || "Flexible";
  ...
  imgWrap.innerHTML = `
    <div class="modal-image-placeholder" style="background:${bgColors[service.category] || "linear-gradient(135deg,#1e1442,#2d1f6e)"}; height:220px;">
      <span style="font-size:5rem;">${service.emoji || "📄"}</span>
    </div>
    <div class="modal-category-badge">${service.category}</div>`;
